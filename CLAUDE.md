...
life-trio is a local-first Android app (single Gradle module `:app`, package `com.lifetrio`) for personal workflows: Memo (notes with tags/images/speech input), Ledger (expense/income tracking with budget), Plan (recurring plans incl. China legal-workday rules), and a biometric-locked Password vault. Kotlin + Jetpack Compose + Room. No account system, no cloud sync, no network. UI text is Chinese.
